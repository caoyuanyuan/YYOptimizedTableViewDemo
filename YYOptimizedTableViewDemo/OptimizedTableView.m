...
@interface OptimizedTableView() <UITableViewDelegate, UITableViewDataSource>
...
- (void)drawCell:(RichTableViewCell *)cell withIndexPath:(NSIndexPath *)indexPath{
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    RichTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell"];
    if (cell==nil) {
        cell = [[RichTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault
...
- (void)loadContent{
    ...
    if (self.visibleCells&&self.visibleCells.count>0) {
        for (id temp in [self.visibleCells copy]) {
            RichTableViewCell *cell = (RichTableViewCell *)temp;
            [cell draw];
        }
    }
}
...
- (void)removeFromSuperview{
    for (UIView *temp in self.subviews) {
        for (RichTableViewCell *cell in temp.subviews) {
            if ([cell isKindOfClass:[RichTableViewCell class]]) {
                [cell releaseMemory];
            }
        }
    }
